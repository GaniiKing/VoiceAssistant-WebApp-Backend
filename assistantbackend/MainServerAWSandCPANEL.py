...
class MyAssistant:
    ...
    def preprocess_query(self, sentence_test):
        ...
        browser_open = "open youtbe.com" 
        news_teller = "Tell me news"     
        turn_sentence = "turn on analyser"   
    ...
    def get_sentence_embedding(self, sentence):
        url = f"http://13.201.95.237/get_sentence_embedding/{sentence}"
    ...
    def check_query_for_urls(self, query_str):
        url = f"http://13.201.95.237/checkForURLs/{query_str}"
        response = requests.get(url)
        print(response.status_code)
        urls = response.json()
        print(urls)
    ...
    def open_book(self,query):
        url = f"http://13.201.95.237/bookNameExtract/{query}"
        response = requests.get(url)
        filtered_sentence = response.json()
        return f"https://openlibrary.org/search?q={filtered_sentence}&mode=everything"
    

    def analyse_and_search(self,query):
        url = f"http://13.201.95.237/analyseandSearchList/{query}"
        response = requests.get(url)
