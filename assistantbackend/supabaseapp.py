...
def search_query_product_name():
    ...
    print(query)
    response = supabase.table('products').select('name').ilike('name', f'%{query}%').order('priority_score', desc=True).order('priority', desc=True).execute()
    if response:
...
def get_CategoryProducts():
    data = request.get_data(as_text=True)
    parsed_data = json.loads(data)
    mainCategory = parsed_data.get('category')
    response = supabase.table('products').select().eq('mainCategory', mainCategory).order('priority_score', desc=True).order('priority', desc=True).execute()
    if response:
        details = response.data
        return jsonify(details),200
    else:
